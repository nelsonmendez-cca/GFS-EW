...
• NUEVO: Módulo de Gráficos Promedio Nacional (Precipitación y Temperatura GFS vs ECMWF vs Ensamble) y descarga en Excel.
"""
...
NORM_PRECIP_DIARIO = mcolors.BoundaryNorm(
    BOUNDS_PRECIP_DIARIO, ncolors=len(colores_rgb), extend="max"
)

BOUNDS_PRECIP_SEMANAL = [0, 5, 10, 20, 30, 50, 75, 100, 150, 200, 250]
NORM_PRECIP_SEMANAL = mcolors.BoundaryNorm(
    BOUNDS_PRECIP_SEMANAL, ncolors=len(colores_rgb), extend="max"
)
...
ESTILOS_MAPA = {
    ...
    "temperature_2m_max": {
        ...
        "label_semanal": "Temp. Máx Promedio (°C)",
    ...
    "temperature_2m_min": {
        ...
        "norm_semanal": None,
        "label_diario": "Temperatura (°C)",
        "label_semanal": "Temp. Mín Promedio (°C)",
...
def cargar_estaciones_ampliadas(ruta: str, buffer_deg: float) -> tuple:
    ...
    for lon_b in lons_ext:
        for lat_b in lats_ext:
            if not (
                min_lon <= lon_b <= max_lon and min_lat <= lat_b <= max_lat
            ):
                estaciones[f"BUFFER_{idx_b}"] = {
                    "name": f"Punto Borde {idx_b}",
                    "lat": float(lat_b),
                    "lon": float(lon_b),
                }
                idx_b += 1
...
def fetch_openmeteo_batch(
    lats: List[float], lons: List[float], modelo: str
) -> List[Dict[str, Any]]:
    params = {
...
def obtener_hillshade_real_recortado(
    grid_lon_mesh: np.ndarray,
    grid_lat_mesh: np.ndarray,
    ...
    try:
        ...
        for i in range(0, len(locations), 100):
            res = requests.post(
                url, json={"locations": locations[i : i + 100]}, timeout=15
            )
            if res.status_code == 200:
                elevations.extend([
                    r["elevation"] for r in res.json()["results"]
                ])
            else:
                break
        ...
        if len(elevations) == len(lons):
            ...
            shaded = np.sin(np.pi / 4.0) * np.sin(slope) + np.cos(
                np.pi / 4.0
            ) * np.cos(slope) * np.cos(3.0 * np.pi / 4.0 - aspect)
            shaded_norm = (shaded - shaded.min()) / (
                shaded.max() - shaded.min() + 1e-5
            )

            geometrias = [geom for geom in gdf_boundary.geometry]
            hill_recortado, _, _ = recortar_y_guardar_raster(
                shaded_norm,
                "hillshade_temp",
                transform,
                height,
...
def interpolar_suave(
    points: np.ndarray,
    values: np.ndarray,
    grid_lon_mesh: np.ndarray,
    grid_lat_mesh: np.ndarray,
    es_precip: bool = False,
) -> np.ndarray:
    try:
        ...
        grid_z = griddata(
            points, values, (grid_lon_mesh, grid_lat_mesh), method="cubic"
        )

    nan_mask = np.isnan(grid_z)
    if nan_mask.any():
        grid_z_near = griddata(
            points, values, (grid_lon_mesh, grid_lat_mesh), method="nearest"
        )
...
def recortar_y_guardar_raster(
    ...
    with rasterio.open(ruta_tif, "r+") as src:
        out_image, out_transform = mask(
            src, geometrias, crop=True, nodata=np.nan
        )
...
def generar_figura_semanal(
    raster_resumen: np.ndarray,
    extent: List,
    gdf_boundary: gpd.GeoDataFrame,
    ...
    plt.title(
        f" {estilo['title']} \n{titulo_semana}: del {f_inicio} al {f_fin}",
        fontsize=12,
        fontweight="bold",
        pad=10,
    )
...
def generar_collage_16_dias(
    ...
    for idx, fecha in enumerate(fechas):
        ...
        f_obj = datetime.datetime.strptime(
            limpiar_fecha_str(fecha), "%Y-%m-%d"
        )
        ax.set_title(
            f_obj.strftime("%d/%m/%Y"), fontsize=9, fontweight="bold"
        )
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    if last_im:
        ...
        if estilo.get("ticks_diario"):
            fig.colorbar(
                ...
                ticks=estilo["ticks_diario"],
                label=estilo.get("label_diario", var),
            )
        else:
            fig.colorbar(
                last_im,
    ...
    fig.suptitle(
        f"Evolución Diaria (16 Días) - El Salvador: {estilo['title']}",
        fontsize=14,
        fontweight="bold",
        y=0.98,
    )
...
def ejecutar_procesamiento():
    estaciones, gdf_boundary = cargar_estaciones_ampliadas(
        ARCHIVO_ESTACIONES, BUFFER_GRADOS
    )
    if not estaciones:
        return
    ...
    for idx_mod, (model_key, alias) in enumerate(MODELOS.items()):
        progreso.progress(
            10 + idx_mod * 20, text=f"Descargando modelo {alias}..."
        )
        for i in range(0, len(items_estaciones), BATCH_SIZE):
            ...
            batch_meta = [
                {
                    "id": est_id,
                    "name": str(meta.get("name", est_id)),
                    "lat": float(meta["lat"]),
                    "lon": float(meta["lon"]),
                }
                for est_id, meta in chunk
            ]
            try:
    ...
    df_puntos_reales = df_raw_all[
        ~df_raw_all["ID"].astype(str).str.startswith("BUFFER_")
    ]
    st.session_state["df_raw_all"] = df_puntos_reales

    # Ensamble promedio
    df_ensamble = (
        df_raw_all.groupby(["ID", "NAME", "lat", "lon", "date"])[DAILY_VARS]
        .mean()
        .reset_index()
    )
    ...
    transform = from_bounds(
        min_lon - 0.1, min_lat - 0.1, max_lon + 0.1, max_lat + 0.1, width, height
    )
    ...
    hillshade_real = obtener_hillshade_real_recortado(
        grid_lon_mesh, grid_lat_mesh, gdf_boundary, transform, height, width
    )
    ...
    for idx_var, var in enumerate(VARIABLES_EXPORTAR):
        progreso.progress(
            50 + idx_var * 15, text=f"Generando rasters de: {var}"
        )
        ...
        for fecha in fechas_disponibles:
            ...
            points, values = (
                df_fecha[["lon", "lat"]].values,
                df_fecha[var].values,
            )

            grid_z = interpolar_suave(
                points,
                values,
                grid_lon_mesh,
                grid_lat_mesh,
                es_precip=(var == "precipitation_sum"),
            )
            fecha_str = limpiar_fecha_str(fecha)
            raster_dia, extent_final, _ = recortar_y_guardar_raster(
                grid_z,
                f"pronostico_ENSAMBLE_{var}_{fecha_str}",
                transform,
                height,
                width,
                geometrias,
        ...
        for nom_sem, grp_fechas in [
            ...
            if grp_fechas:
                ...
                df_sem_agg = (
                    df_sub_sem.groupby(["ID", "NAME", "lat", "lon"])[var]
                    .sum()
                    .reset_index()
                    if var == "precipitation_sum"
                    else df_sub_sem.groupby(["ID", "NAME", "lat", "lon"])[var]
                    .mean()
                    .reset_index()
                )

                points_sem, values_sem = (
                    df_sem_agg[["lon", "lat"]].values,
                    df_sem_agg[var].values,
                )
                grid_z_sem = interpolar_suave(
                    points_sem,
                    values_sem,
                    grid_lon_mesh,
                    grid_lat_mesh,
                ...
                raster_sem, extent_final, _ = recortar_y_guardar_raster(
                    grid_z_sem,
                    f"pronostico_ENSAMBLE_{var}_{nom_sem}",
                    transform,
                    height,
                    width,
                    geometrias,
    ...
    st.success(
        "🎉 Datos procesados. Ya puedes ver los mapas, gráficos promedio y"
...
def render_graficos_promedio():
    ...
    if "df_raw_all" not in st.session_state:
        st.warning(
            "⚠️ Por favor haga clic en '🔄 Actualizar Datos / Procesar' en la"
            " barra lateral para generar la información."
        )
        return
    ...
    df_prom_diario = (
        df_raw.groupby(["modelo", "date"])[DAILY_VARS].mean().reset_index()
    )
    df_prom_diario["temperature_2m_mean"] = (
        df_prom_diario["temperature_2m_max"]
        + df_prom_diario["temperature_2m_min"]
    ) / 2.0

    fechas = sorted(df_prom_diario["date"].unique())

    # DataFrames Pivotados
    piv_rain = df_prom_diario.pivot(
        index="date", columns="modelo", values="precipitation_sum"
    ).reset_index()
    piv_tmax = df_prom_diario.pivot(
        index="date", columns="modelo", values="temperature_2m_max"
    ).reset_index()
    piv_tmin = df_prom_diario.pivot(
        index="date", columns="modelo", values="temperature_2m_min"
    ).reset_index()
    piv_tmean = df_prom_diario.pivot(
        index="date", columns="modelo", values="temperature_2m_mean"
    ).reset_index()
    ...
    fig_rain.update_layout(
        barmode="group",
        xaxis_title="Fecha",
        yaxis_title="Precipitación Promedio (mm)",
        hovermode="x unified",
    )
    ...
    fig_ens.add_trace(
        go.Scatter(
            x=piv_tmean["date"],
            y=piv_tmean["Ensamble"],
            name="T. Media Ensamble",
            mode="lines+markers",
    ...
    df_rain_export = piv_rain.rename(
        columns={
            "ECMWF": "Lluvia ECMWF (mm)",
            "GFS": "Lluvia GFS (mm)",
            "Ensamble": "Lluvia Ensamble (mm)",
        }
    )
    ...
    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
        df_rain_export.to_excel(
            writer, sheet_name="Precipitacion_Promedio", index=False
        )
        df_temp_export.to_excel(
            writer, sheet_name="Temperaturas_Promedio", index=False
        )
...
if "datos_procesados" in st.session_state:
    ...
    with tab1:
        semana = st.radio(
            "Selecciona Período Semanal (7 Días):",
            ["Semana 1", "Semana 2"],
            horizontal=True,
        )
        ...
        grupo_fechas = (
            datos_var["fechas_s1"]
            if semana == "Semana 1"
            else datos_var["fechas_s2"]
        )

        if grupo_fechas and key_sem in datos_var.get("raster_semanal", {}):
            raster_resumen = datos_var["raster_semanal"][key_sem]
            f_init_str = datetime.datetime.strptime(
                limpiar_fecha_str(grupo_fechas[0]), "%Y-%m-%d"
            ).strftime("%d de %B")
            f_end_str = datetime.datetime.strptime(
                limpiar_fecha_str(grupo_fechas[-1]), "%Y-%m-%d"
    ...
    st.download_button(
        label=(
            "⬇️ Descargar Paquete GeoTIFFs (.ZIP) -"
            f" {ESTILOS_MAPA[var_seleccionada]['title']}"
        ),
        data=bytes_zip,
