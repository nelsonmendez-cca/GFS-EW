...
def generar_collage_16_dias(raster_dict: Dict[str, np.ndarray], extent: List, gdf_boundary: gpd.GeoDataFrame, hillshade: np.ndarray, var: str) -> plt.Figure:
    """ Restaura la generación de la cuadrícula 4x4 de 16 días. """
    ...
    for idx, fecha in enumerate(fechas):
        ...
        f_obj = datetime.strptime(limpiar_fecha_str(fecha), "%Y-%m-%d")
        ax.set_title(f_obj.strftime("%d/%m/%Y"), fontsize=9, fontweight='bold')
        ax.set_xticks([])
        ax.set_yticks([])

    # Rellenar ejes vacíos si hay menos de 16 días
...
def crear_zip_tiffs(var_seleccionada: str) -> bytes:
    """ Empaca todas las capas GeoTIFF en memoria para habilitar el botón de descarga. """
...
def ejecutar_procesamiento():
    ...
    st.success("🎉 Datos procesados. Ya puedes ver los collages y descargar los GeoTIFFs.")
...
if 'datos_procesados' in st.session_state:
    ...
    tab1, tab2 = st.tabs(["📅 Resumen Semanal", "🗓️ Collage 16 Días"])
